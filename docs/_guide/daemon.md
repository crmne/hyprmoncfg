...
### Optional opt-out of lid handling

Start the daemon with `--disable-lid` to turn lid handling off entirely: the
lid switch is never read, lid changes are never watched, and the closed-lid
policy never runs. The daemon behaves as if the machine had no lid, so the
saved profile for the attached displays is applied and left alone when the lid
closes. This suits setups that drive their own DPMS-based clamshell handler
(see issue #72 for the remove/add apply loop this avoids). The option is off by
default, and `--lid-poll-interval` has no effect while it is set.
...
hyprmoncfgd --disable-lid         # never read or react to the lid switch
