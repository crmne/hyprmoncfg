...
func renderLuaConfig(p profile.Profile, monitors []hypr.Monitor, useV2 bool) (string, error) {
	...
	for _, item := range matched {
		...
		identifier := resolver.SelectorForOutput(item.config, item.monitor)
		if useV2 {
			monitorBlocks = append(monitorBlocks, renderLuaMonitorV2Block(identifier, item.config, mirrorTarget))
			continue
		}
	...
	for _, rule := range rules {
		...
		selector := resolver.SelectorForOutput(output, monitor)
		workspaceLines = append(workspaceLines, "hl.workspace("+luaQuote(workspaceRuleCommand(rule.Workspace, selector, rule.Default, rule.Persistent))+")")
	}
...
func renderLuaMonitorV2Block(identifier string, output profile.OutputConfig, mirrorTarget string) string {
	lines := []string{
		"hl.monitorv2 {",
